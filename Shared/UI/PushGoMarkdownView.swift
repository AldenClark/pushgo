...
private struct MarkdownTableView: View {
    ...
    private func cellText(_ inlines: [MarkdownInline]) -> some View {
        Text(attributedStringForTable(inlines: inlines))
            .lineLimit(nil)
...
private func attributedStringForTable(inlines: [MarkdownInline]) -> AttributedString {
    inlines.reduce(into: AttributedString()) { result, inline in
        result += attributedStringForTable(inline: inline)
    }
}

private func attributedStringForTable(inline: MarkdownInline) -> AttributedString {
    switch inline {
    case let .text(text):
        return AttributedString(insertBreaks(text))
    case let .bold(inlines):
        var string = attributedStringForTable(inlines: inlines)
    ...
    case let .highlight(inlines):
        ...
        return string
    case let .code(text):
        var string = AttributedString(insertBreaks(text))
    ...
    case let .link(text: linkText, url: urlString):
        var string = attributedStringForTable(inlines: linkText)
    ...
    case let .mention(value):
        var string = AttributedString(insertBreaks("@\(value)"))
        string.foregroundColor = .accentColor
        return string
    case let .tag(value):
        var string = AttributedString(insertBreaks("#\(value)"))
    ...
    case let .autolink(link):
        var string = AttributedString(insertBreaks(link.value))
...
private func insertBreaks(_ text: String, maxRun: Int = 12) -> String {
    guard maxRun > 0 else { return text }
    var result = ""
    var runCount = 0
    for char in text {
